     ...
     (toEnv
       (setPstate
         (setVarBool
           ...
           lock' SUFFICIENT')
         Washing' Washing'locking'))"



theorem "VC10 inv2 env s0 startButton'value locked'value waterLevel'value waterPresence'value temp'value freq'value"
  apply(unfold VC10_def inv2_def R2_def)
  apply(rule impI)
  apply(rule conjI)
   apply(rule conjI)
    apply simp
  ...
  using extra10 by(auto simp add: VC10_def)
