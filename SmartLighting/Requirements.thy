...
(\<forall> s1. substate s1 s \<and> toEnvP s1 \<and> toEnvNum s1 s \<ge> LIGHTING_TIME'TIMEOUT - 1 \<and> getVarBool s1 presence' = False \<longrightarrow>
(\<exists> s3. toEnvP s3 \<and> substate s1 s3 \<and> substate s3 s \<and> toEnvNum s1 s3 \<le> LIGHTING_TIME'TIMEOUT \<and>
