...
lemma P4inv_rule: "
P1 \<longrightarrow> P4inv s0 t t1 A1 A2 A3 \<Longrightarrow> 
P2 \<and> A1 s0 s \<longrightarrow> A2 s \<or> A3 s \<and> t2 > 0  \<Longrightarrow>
...
P2 \<longrightarrow> P4inv s t t2 A1 A2 A3"
  apply(rule impI)
  apply simp
...
lemma P4inv_rule2: "
 ...
 P4inv s t t2 A1 A2 A3"
  ...
  subgoal for s1 s2
    ...
    apply (metis One_nat_def Suc_eq_plus1 Suc_leI dual_order.trans substate_refl toEnvNum3)
    by (metis One_nat_def Suc_eq_plus1 less_trans_Suc toEnvNum3)
  by (metis (full_types) add_is_1 nat_1 nat_one_as_int substate_linear substate_toEnvNum_id toEnvNum3)
