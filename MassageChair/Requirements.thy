theory Requirements
  imports MassageChair "../Pattern1_Def" "../Pattern6_Def" "../Pattern12_Def" "../Pattern3_Def"
begin
...
P3 s (\<lambda> s2'.  P12sub s2' ON_OFF_TIME'TIMEOUT (\<lambda> s1. getVarBool s1 enabled' = True) (\<lambda> s2. getVarBool s2 onOff' \<noteq> PRESSED')
 (\<lambda> s3. getVarBool s3 onOff' = PRESSED'))
  (\<lambda> s2. getVarBool s2 roller' = False \<and> getVarBool s2 vibration' = False)"
...
P6_1 s (\<lambda> s2. P12sub s2 ON_OFF_TIME'TIMEOUT (\<lambda> s1. getVarBool s1 enabled' = True) (\<lambda> s2'. getVarBool s2' onOff' \<noteq> PRESSED') (\<lambda> s3. getVarBool s3 onOff' = PRESSED'))
(\<lambda> s4. getVarBool s4 up' = True)
(\<lambda> s5. getVarBool s5 upper' = True)"
